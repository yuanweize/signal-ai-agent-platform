...
def build_generation_prompt_messages(state: AgentState) -> list[dict[str, Any]]:
    """Build standardized prompt messages with all context layers and multimodal inputs."""
    ...
    if memories:
        ...
        messages.append({"role": "system", "content": f"Customer Profile & Memory:\n{mem_text}"})
    ...
    # Inject current user message along with multimodal attachment context (untrusted user content)
    ...
    attachment_ctx = state.get("attachment_context")
    if attachment_ctx:
        current_user_msg = f"{current_user_msg}\n\n[Attachment Content (untrusted customer input)]:\n{attachment_ctx}".strip()
...
async def select_skills_step(state: AgentState) -> dict[str, Any]:
    ...
    if not is_policy_inquiry and any(
        w in text for w in ["broken", "defect", "refund", "complaint", "terrible", "late", "damage"]
    ):
        candidate_skills.append("complaints")
    elif "refund" in text:
        candidate_skills.append("complaints")
    if any(w in text for w in ["human", "agent", "person", "representative", "manager", "staff"]):
...
async def retrieve_knowledge_step(
    state: AgentState, retriever: KnowledgeRetriever | None
) -> dict[str, Any]:
...
async def plan_tools_step(state: AgentState, llm: LLMProvider) -> dict[str, Any]:
...
async def generate_response_step(state: AgentState, llm: LLMProvider) -> dict[str, Any]:
...
def build_agent_graph(
    ...
    async def retrieve_knowledge_node(s: AgentState) -> dict[str, Any]:
        return await retrieve_knowledge_step(s, retriever)

    async def plan_tools_node(s: AgentState) -> dict[str, Any]:
        return await plan_tools_step(s, llm)

    async def generate_response_node(s: AgentState) -> dict[str, Any]:
        return await generate_response_step(s, llm)

    workflow = StateGraph(AgentState)
    workflow.add_node("select_skills", select_skills_step)
